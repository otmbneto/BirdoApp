...
class uiItem(QtGui.QGroupBox):
    ...
    def __init__(self,fullpath,episode_list,decimal_cb):
        ...
        self.regex_sc_ep = r'^(EP|SC(N?)|SQ)?\d+$'
    ...
    def getShot(self,filename):
        ...
        if len(result) != 2:
            print 'Invalid file name!'
            return None
        return 'SC{:04d}'.format(result[1] * 10) if self.decimal_checkbox.isChecked() else 'SC{:04d}'.format(result[1])
    ...
    def upload_scene(self,root,project_folders,project_data,temp):
        ...
        script_name = "BAT_birdofy_decimal.js" if self.decimal_checkbox.isChecked() else "BAT_birdofy_normal.js"
