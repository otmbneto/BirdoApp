...
function isAssetTemplate(){//checa se o arquivo aberto e o ASSET template ou se ja e um arquivo de ASSET
	return scene.currentScene() == "ASSET_template";
}
...
function createInterface(ui_path, initialFolder, jsonInfo){//inicia a interface pra escolher o nome do ASSET
	...
	var prefixos = ["", "CHAR", "PROP", "FX", "MISC"];//FIXME:SE precisar adicionar Prefixo, adicione aqui!
...
///CALLBACK FUNCTIONS///
	var updateName = function(){
		...
		if(type == "MI"){
			ui.spinBox.enable = false;	
			number = "";
		}
		
		var name = ui.lineEdit.text;
		if(!name){
...
////////////////////FUNCOES EXTRAS INTERFACE//////////////
	...
	function updateSetup(assetName, updatePallet){//muda o export e o nome do node (pergunta sobre palette)
		...
		var assetNode = node.root() + "Top/Asset";
		var assetNodePeg = node.root() + "Top/Asset-P";
		//rename read node
		...
		//rename peg
		node.rename(assetNodePeg, "assetName" + "-P");
		
		//change export
		var exportNode = node.root() + "/SETUP/EXPORT";
