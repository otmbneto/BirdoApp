/*
{VERSAO ADAPTADA PARA RIGS DO LUPI&BADUKI}
...
function loadInterpolationStates(){
  ...
  //modificacao SlIDER birdo
  if("files" in uiData && "node_ref" in uiData){
	var ref_node = uiData.node_ref.replace("~", node.parentNode(Controller.node));//node drawing com info do da pose do TURN
	var col = node.linkedColumn(ref_node, "DRAWING.ELEMENT");
	var poseLetter = column.getEntry(col, 1, frame.current());
	if(poseLetter == "" || poseLetter == "Zzero" || !uiData.files.hasOwnProperty(poseLetter)){
		MessageLog.trace("No slider for current pose! Hiding slider!");
		node.showControls(Controller.node, false);
		return;
	}
	var filep = uiData.files[poseLetter];
	uiData["poses"] = filep;
  }
...
//update frame callback adaptado
